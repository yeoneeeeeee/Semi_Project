...
<head>
	<meta charset="UTF-8">
	<title>7H THE TOWN | 게시판(나눔)</title>
        ...
        <div class="board_list_wrap_body col s12 m2 z-depth-2">
            <div class="board_list_wrap">
                <div class="board_list">
                    ...
                    <%for(Board b : list){ %>
                    <%if(b.getBoardCategory().equals("나눔게시판")){ %>
			...
			<div style="display:flex; justify-content:center; margin-top:50px; margin-bottom:200px;" >
				...
				<% if(currentPage != 1)  {%>
				    <li class="disabled"><a href="<%=contextPath %>/list.yo?currentPage=<%=currentPage -1 %>"><i class="material-icons">chevron_left</i></a></li>
			    <% } %>
			    
			   	<% for(int p = startPage; p <= endPage; p++) { %>
			    <% if(p != currentPage){ %>
				    <li class="waves-effect"><a href="<%=contextPath%>/list.yo?currentPage=<%= p %>"><%= p %></a></li>
			    <% } else { %>
			    	<li class="active"><a href="<%=contextPath%>/list.yo?currentPage=<%= p %>"><%= p %></a></li>
			   	<% } %>
				<% } %>
				<% if(maxPage == 0) maxPage=1; %>
				<% if(currentPage != maxPage) { %>
				    <li class="waves-effect"><a href="<%=contextPath%>/list.yo?currentPage=<%= currentPage + 1 %>"><i class="material-icons">chevron_right</i></a></li>
				</ul>
				<% } %>
			</div>
		<script>
