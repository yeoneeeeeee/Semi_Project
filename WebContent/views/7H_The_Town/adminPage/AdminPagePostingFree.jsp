...
<link rel="stylesheet"
	href="views/7H_The_Town/resource/CSS,JS/adminPage/AdminPagePostingFree.css">
...
<title>7H THE TOWN | 게시판관리(일반)</title>
</head>
		...
		<div class="container">
			...
			<div class="adminpage-content">
				...
				<div style="display:flex; justify-content:center;" >
					...
					<% if(currentPage != 1)  {%>
					    <li class="disabled"><a href="<%=contextPath %>/free.ad?currentPage=<%=currentPage -1 %>&category="><i class="material-icons">chevron_left</i></a></li>
				    <% } %>
				    
				    <% for(int p = startPage; p <= endPage; p++) { %>
				    <% if(p != currentPage){ %>
					    <li class="waves-effect"><a href="<%=contextPath%>/free.ad?currentPage=<%= p %>"><%= p %></a></li>
				    <% } else { %>
					...
					<% if(currentPage != maxPage) { %>
					    <li class="waves-effect"><a href="<%=contextPath%>/free.ad?currentPage=<%= currentPage + 1 %>"><i class="material-icons">chevron_right</i></a></li>
					</ul>
					<% } %>
				</div>
				<% } else { %>
