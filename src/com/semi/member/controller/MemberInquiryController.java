...
public class MemberInquiryController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(loginUser != null) {			
			...
			FAQ faq = (FAQ) new BoardService().selectCountFaqBoard(userId);
			int boardAsString = 0;
			if(faq != null) {				
				boardAsString = faq.getCount();
			}
			...
			ArrayList<FAQ> list = new BoardService().selectBoardFaqMember(userId,pi);
	
			request.setAttribute("list", list);
			
			request.getRequestDispatcher("/views/7H_The_Town/mypage/MyPageInquiry.jsp").forward(request, response);
