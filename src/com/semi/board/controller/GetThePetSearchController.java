...
/**
 * Servlet implementation class GetThePetSearchController
 */
@WebServlet("/search.get")
public class GetThePetSearchController extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public GetThePetSearchController() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		listCount = new GetThePetService().selectSearchListCount(keyword);
		...
		ArrayList<GetThePet> gList = new GetThePetService().searchGetThePetList(pi,keyword);
		ArrayList<Pet> pList = new GetThePetService().searchGetThePetInfoList(pi,keyword);
		...
		if(gList != null && gList.size() != 0) {
			...
			request.getSession().setAttribute("alertMsg", "검색결과가 없습니다.");
